...
public class GalleryFinal {
    ...
    static final int REQUEST_CODE_GALLERY_ICON = 1003;

    static final int PERMISSIONS_CODE_GALLERY = 2001;

    static final int PERMISSIONS_CODE_OPEN_CAMER = 3001;

    static final int PERMISSIONS_CODE_OPEN_VIDEO = 4001;
    ...
    public static void openGallerySingle(int requestCode, FunctionConfig config, OnHanlderResultCallback callback) {
        ...
        if(mCoreConfig.getContext() != null){
            Intent intent = new Intent(mCoreConfig.getContext(), PhotoSelectActivity.class);
            if (!(mCoreConfig.getContext() instanceof Activity)) {
                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            }
            mCoreConfig.getContext().startActivity(intent);
        }else {
            Toast.makeText(mCoreConfig.getContext(), R.string.photo_list_empty, Toast.LENGTH_SHORT).show();
        }
    }
