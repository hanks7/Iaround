...
public class Bytes
{
	...
	{
		return new StringBuilder( ).append( toString( b1 , 0 , b1.length ) ).append( sep )
				.append( toString( b2 , 0 , b2.length ) ).toString( );
	}
		...
		try
		{
			...
			for ( int i = 0 ; i < first.length( ) ; i++ )
			{
						...
						|| ( " `~!@#$%^&*()-_=+[]{}\\|;:'\",.<>/?".indexOf( ch ) >= 0 ) )
				{
					...
					result.append( String.format( "\\x%02X" , new Object[ ]
						{ Integer.valueOf( ch ) } ) );
			}
		}
		catch ( UnsupportedEncodingException e )
	...
	public static byte[ ] toBytes( Serializable object )
	{
		...
		catch ( IOException e )
		{
			throw new IllegalArgumentException(
					"the object not implement Serializable interface." );
		}
		finally
		{
			if ( byteOut != null )
	...
	public static Object toObject(byte[ ] bytes ) throws ClassNotFoundException
	{
		...
		finally
		{
			if ( byteIn != null )
				...
				{
					LOG.log( Level.WARNING , "error closing ByteArrayInputStream" , e );
				}
	...
	public static byte[ ] incrementBytes( byte[ ] value , long amount ) throws IOException
	{
		...
		else if ( val.length > 8 )
		{
			throw new IllegalArgumentException( new StringBuilder( )
					.append( "Increment Bytes - value too big: " ).append( val.length )
					.toString( ) );
		}
