...
public class JazzyViewPager extends ViewPager
{
	...
	public JazzyViewPager(Context context , AttributeSet attrs )
	{
		...
		setOutlineEnabled( ta.getBoolean( R.styleable.JazzyViewPager_jzOutlineEnabled , false ) );
		setOutlineColor( ta.getColor( R.styleable.JazzyViewPager_jzOutlineColor , Color.WHITE ) );
		switch ( mEffect )
		{
			case Stack :
			case ZoomOut :
	...
	private void logState(View v , String title )
	{
		Log.v( TAG ,
				title + ": ROT (" + ViewHelper.getRotation( v ) + ", "
						+ ViewHelper.getRotationX( v ) + ", " + ViewHelper.getRotationY( v )
						+ "), TRANS (" + ViewHelper.getTranslationX( v ) + ", "
						+ ViewHelper.getTranslationY( v ) + "), SCALE ("
						+ ViewHelper.getScaleX( v ) + ", " + ViewHelper.getScaleY( v )
	...
	private void animateZoom(View left , View right , float positionOffset , boolean in )
	{
		if ( mState != State.IDLE )
		{
			...
			{
				manageLayer( right , true );
				mScale = in ? ZOOM_MAX + ( 1 - ZOOM_MAX ) * positionOffset : 1 + ZOOM_MAX
						- ZOOM_MAX * positionOffset;
				ViewHelper.setPivotX( right , right.getMeasuredWidth( ) * 0.5f );
				ViewHelper.setPivotY( right , right.getMeasuredHeight( ) * 0.5f );
