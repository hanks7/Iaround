...
public class NearActivity extends TitleActivity implements HttpCallBack, LocationUtil.MLocationListener, View.OnClickListener, MainFragmentActivity.PagerSelectNear {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setTitle_C(R.string.near_personal);
        setContent(R.layout.main_nearby_view);
        emptyView = LayoutInflater.from(this).inflate(R.layout.no_data_view, null);
        initViews();
        initData();
        setListeners();
        initAnimation();
    ...
    private void initViews() {
        ...
        mIvRight_1 = (ImageView) findViewById(R.id.iv_right);
        mIvRight_1.setVisibility(View.VISIBLE);
        mIvRight_1.setImageResource(R.drawable.near_filter);
        ...
        initNearUserListView();
        mNearUserListView.getRefreshableView().addHeaderView(headerView);

        mSwitchBtn = (Button) findViewById(R.id.switch_mode_btn);
     ...
     * 初始化附近用户列表View
     */
    private void initNearUserListView() {
        if (mNearUserListView == null) {
            mNearUserListView = (PullToRefreshListView) findViewById(R.id.nearby_user_listview);
    ...
    private class NearbyListAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            switch (itemViewType) {
                ...
                case ACTIVITY_RECOMMENDS:
                    break;
                default:
                    NearbyListAdapter.NearHolder holder = null;
                    if (convertView == null) {
                        holder = new NearbyListAdapter.NearHolder();
                        ...
                        holder = (NearbyListAdapter.NearHolder) convertView.getTag();
                    }
                    ...
                    if (TextUtils.isEmpty(name)) {
                        if (!"".equals(item.user.notes) && item.user.notes != null) {
                            spName = FaceManager.getInstance(NearActivity.this).parseIconForString(NearActivity.this, item.user.notes, 0, null);
                        } else {
                            if (!TextUtils.isEmpty(item.user.nickname)) {
                                spName = FaceManager.getInstance(NearActivity.this).parseIconForString(NearActivity.this, item.user.nickname, 0, null);
                            } else {
                                spName = FaceManager.getInstance(NearActivity.this).parseIconForString(NearActivity.this, String.valueOf(item.user.userid), 0, null);
                            }
                        }
        ...
        public void onClickUser(NearByExtendUser neu) {
            NearByExtendUser.NearByUser u = neu.user;
            ...
            if (user.getUid() == Common.getInstance().loginUser.getUid()) {
                Intent intent = new Intent(NearActivity.this, UserInfoActivity.class);
                intent.putExtra(Constants.UID, user.getUid());
                intent.putExtra("user", Common.getInstance().loginUser);
                startActivityForResult(intent, 201);
            } else {
                if (neu.getGname() == null) {
                    Intent intent = new Intent(NearActivity.this, OtherInfoActivity.class);
                    intent.putExtra(Constants.UID, user.getUid());
