...
public class FaceDetailActivity extends BaseFragmentActivity implements OnTouchListener,HttpCallBack

{
	...
	private final int DOWNDLOAD_BACKGROUND = 901;// 在表情中心详情下载背景图片
	private final int DOWNDLOAD_IMAGE = 902;// 在表情中心详情下载标题图片
...
//			HashMap< String , Object > response = FaceCenterModel.getInstance( FaceDetailActivity.this ).getRes( result ,
//					flag );
//			if ( response.isEmpty( ) )
//			{
//				return;
//			}
//			FaceCenterModel.FaceCenterReqTypes reqType = ( FaceCenterReqTypes ) ( response.get( "reqType" ) );
...
//			FaceCenterReqTypes reqType = FaceCenterModel.getInstance( FaceDetailActivity.this ).getReqType( flag );
//			if ( reqType == FaceCenterReqTypes.FaceDetailData )
//			{
...
//	};
		...
		public void handleMessage( Message msg )
		{
			...
			switch ( msg.what )
			{
				...
				case UPDATE_PRECENT ://在表情中心或我的表情页下载表情时，更新view
					...
					break;
				case DOWNDLOAD_BACKGROUND://在表情中心下载背景图片，更新view
					displayDownloadBackgroud((BitmapDrawable) msg.obj);
					break;
				case DOWNDLOAD_IMAGE://在表情中心下载标题图片，更新view
					displayDownloadImage((BitmapDrawable) msg.obj);
					break;
			}
		}
	}
	...
	private void initNetDataFail( String result )
	{
		if ( !TextUtils.isEmpty( result ) )
		{
			...
			{
				ErrorCode.showError( FaceDetailActivity.this , result );
			}
		}else {
...
//				authorImage , R.drawable.default_avatar_round_light, R.drawable.default_avatar_round_light, null );
	...
	class MyGridViewAdapter extends BaseAdapter
	{
		...
		public View getView( int position , View convertView , ViewGroup parent )
		{
			...
			if ( convertView == null )
			{
				...
				convertView = LayoutInflater.from( FaceDetailActivity.this ).inflate(
...
//					NetImageView.DEFAULT_SMALL , NetImageView.DEFAULT_SMALL );
			GlideUtil.loadImage(BaseApplication.appContext,imageUri,viewHolder.image, NetImageView.DEFAULT_SMALL , NetImageView.DEFAULT_SMALL);
...
//		pop.setAnimationStyle( R.style.AnimationFade );//淡入淡出效果
	...
	{
//		if ( !OkHttpManager.getInstance( mContext ).CheckNetwork( mContext ) )// 网络未连接
//		{
//			CommonFunction.toastMsg( mContext , getString( R.string.network_req_failed ) );
//			return;
//		}
			...
			new Thread( new Runnable( )
			{
				...
				public void run( )
				{
					FACE_DESCRIBE = BusinessHttpProtocol.getFaceDescribe( mContext ,
							face.getFaceid( ) , FaceDetailActivity.this );

					/** 表情保存路径 */
					String path = PathUtil.getFaceDir( ) + Common.getInstance().getUid( );
	...
	private OnClickListener onClickListener = new OnClickListener( )
	{
		...
		public void onClick( View v )
		{
			...
			else if ( appStatus == -1 )
			{
				if ( face.getOwn( ) == 0 )
				{// 未拥有
//					addBtnEvent( DataTag.BTN_find_face_item_get);
					handleBuyCondition( );
				}
				else
				{
	...
	private void buySuccess( )
	{
		...
		FaceManager.FaceLogoIcon faceLogoIcon = null;
	...
	protected void handleBuyCondition( )
	{
		...
		if ( face.getFeetype( ) == 2 )// VIP免费
		{
			...
			{
				DialogUtil.showTobeVipDialog( FaceDetailActivity.this , R.string.vip_face ,
		...
		{// ** 5.6 NEW **购买类型为打折或者限时免费，价格没有是否VIP之分
			...
			DialogUtil.showTwoButtonDialog( FaceDetailActivity.this ,
				...
				if ( error == 4000 )
				{
					if ( face.getCurrencytype( ) == 1 )
					{
						DialogUtil.showTwoButtonDialog( FaceDetailActivity.this ,
								...
								getString( R.string.face_gold_not_enough ) ,
...
//										FragmentPayBuyGlod.jumpPayBuyGlodActivity(mContext);
					...
					{
						DialogUtil.showDiamondDialog( FaceDetailActivity.this ,"");
					}
					return;
				}
		...
		{
			Toast.makeText( FaceDetailActivity.this , R.string.start_reconnect , Toast.LENGTH_SHORT )
					.show( );
		}
	}
	...
	private void SkipWebView( )
	{
		...
		{
			Toast.makeText( FaceDetailActivity.this , R.string.game_center_ad_game_link_error ,
...
//		finish( );
	...
	{
		Intent i = new Intent( activity , FaceDetailActivity.class );
	...
	private void displayDownloadBackgroud(BitmapDrawable drawable) {
	...
	private void displayDownloadImage(BitmapDrawable drawable) {
		imageView = (ImageView) findViewById(R.id.icon_img);
		imageView.setBackgroundDrawable(drawable);
	}

	private void ImageLoad() {
//        GlideUtil.loadImage(FaceDetailActivityNew.this, face.getBackground(), (ImageView) findViewById(R.id.icon_background));
//       GlideUtil.loadImage(FaceDetailActivityNew.this, face.getImage(), (ImageView) findViewById(R.id.icon_img));

		new Thread(new Runnable() {
			@Override
			public void run() {
				try {
					BitmapDrawable drawable = getDrawable(face.getBackground());
					mMainHandler.sendMessage(mMainHandler.obtainMessage(DOWNDLOAD_BACKGROUND,
							drawable));
				} catch (Exception e) {
					e.printStackTrace();
				}
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					BitmapDrawable drawable = getDrawable(face.getImage());
					mMainHandler.sendMessage(mMainHandler.obtainMessage(DOWNDLOAD_IMAGE,
							drawable));
				} catch (Exception e) {
					e.printStackTrace();
				}
...
//        ImageViewUtil.getDefault().fadeInLoadImageInConvertView(face.getBackground(),
//                (ImageView) findViewById(R.id.icon_background), 0, 0,
...
//                        }
//
//                        ImageView icon = (ImageView) findViewById(R.id.icon_img);
//                        icon.setVisibility(View.GONE);
//                        ImageViewUtil.getDefault().loadImageNoCache(face.getImage(), icon, 0,
...
//                                                    .decomBackGroundPression(mContext, arg2);
//                                            ((ImageView) arg1).setImageBitmap(decomPression);
