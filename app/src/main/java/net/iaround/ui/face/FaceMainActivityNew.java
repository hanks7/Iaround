...
import net.iaround.ui.activity.OwnFaceActivity;
...
public class FaceMainActivityNew extends BaseFragmentActivity implements OnTouchListener, OnClickListener,HttpCallBack
{
	private Dialog pd;
	private long Uid = 0 ;
	...
	{
		Intent i = new Intent( context , FaceMainActivityNew.class );
	...
	private void setListener( )
	{
		...
		mPullListView.setOnItemClickListener( new OnItemClickListener( )
		{
			...
			{
//				addBtnEvent( DataTag.BTN_find_face_item);
				position -= 2;
				...
				if ( clickFace != null )
				{
					...
					FaceDetailActivityNew.launchForResult( FaceMainActivityNew.this ,
							clickFace.getFaceid( ) );
				}
			}
		} );
	...
	protected void requestData( )
	{
		...
		long flag = FaceCenterModel.getInstance( FaceMainActivityNew.this ).getFaceCenterData(
				FaceMainActivityNew.this , 1 , 20 , FaceMainActivityNew.this );
		if ( flag == -1 )
		{
			stopPulling( );
...
//		this.onGeneralSuccess( result , flag );
	}
	...
	private OnClickListener avBannerOnClickListener = new OnClickListener( )
	{
		...
		{
			FaceAd ad = (FaceAd) v.getTag( R.id.im_face_ad );
			if ( ad.getType( ) == FaceAd.TYPE_FACE )
			{
				...
				if ( faceId > 0 )
				{
					FaceDetailActivityNew.launch( FaceMainActivityNew.this , faceId );
				}
			}
			else if ( ad.getType( ) == FaceAd.TYPE_WEBVIEW_URL )
			{
				...
				{
					Toast.makeText( FaceMainActivityNew.this ,
	...
	public void buyFail(String result , Long flag )
	{
		...
		if ( !TextUtils.isEmpty( result ) )
		{
			...
			try
			{
				...
				if ( error == 4000 )
				{
					...
					{
						DialogUtil
								.showDiamondDialog( mActivity , DataTag.VIEW_find_faceCenter );// 获取钻石
					}
					return;
				}
				else if ( error == 5954 )
				...
				{
					DialogUtil.showDiamondDialog( mActivity , DataTag.VIEW_find_faceCenter );// 获取钻石
					return;
				}
				else if ( error == 5953 )
				{
		...
		{
			Toast.makeText( FaceMainActivityNew.this , R.string.start_reconnect , Toast.LENGTH_SHORT )
					.show( );
		}
	}
	...
	{
		Toast.makeText(FaceMainActivityNew.this, "金币不足", Toast.LENGTH_SHORT).show();
//		DialogUtil.showTwoButtonDialog( FaceMainActivityNew.this ,
//				ErrorCode.getErrorMessageId( ErrorCode.E_4000 ) ,
//				getString( R.string.face_gold_not_enough ) , getString( R.string.cancel ) ,
//				getString( R.string.diamond_for_gold_ok ) , null , new View.OnClickListener( )
//				{
//					@Override
//					public void onClick( View v )
//					{
////						FragmentPayBuyGlod.jumpPayBuyGlodActivity(mContext);// 取消金币兑换功能
////						Intent i = new Intent( FaceMainActivity.this , PayMainActivity.class );
////						i.putExtra( PayMainActivity.TAB_INDEX , PayMainActivity.TAB_ONLINE_TIME );
////						i.putExtra( EventBuffer.TAG_NAME , DataTag.VIEW_find_faceCenter );
////						startActivity( i );
//					}
//
//				} );
	...
	{
		DialogUtil.showTwoButtonDialog( FaceMainActivityNew.this , getString( R.string.dialog_title ) ,
				getString( R.string.face_not_participate_active ) , getString( R.string.ok ) ,
				getString( R.string.face_participate_active ) , null , new View.OnClickListener( )
				{
					@Override
					public void onClick( View v )
					{
	...
	private class FaceAdapter extends BaseAdapter
	{
		...
		public View getView(int position , View convertView , ViewGroup parent )
		{
			...
			if ( convertView == null )
			{
				...
				convertView = LayoutInflater.from( FaceMainActivityNew.this ).inflate(
...
//						R.drawable.default_pitcure_small );//jiqiang
				GlideUtil.loadImage(BaseApplication.appContext,face.getIcon(),viewHolder.faceIcon,R.drawable.default_pitcure_small ,
				...
				viewHolder.DownloadLayout.setOnClickListener( new OnClickListener( )
				{
					...
					public void onClick( View v )
					{
						...
						{
//							addBtnEvent( DataTag.BTN_find_face_download);//jiqiang 统计
							viewHolder.DownloadLayout.setClickable( true );
	...
	public void DownLoadFace( final ViewHolder viewHolder , final Face face )
	{
		if ( face.getOwn( ) == 0 )
		{
			...
			if ( face.getFeetype( ) == 2 )
			{
				...
				{
					DialogUtil.showTobeVipDialog( this , R.string.vip_face ,
							R.string.tost_face_vip_privilege, DataTag.VIEW_find_faceCenter);
					return;
				}
			}
	...
	{
		if ( !ConnectorManage.getInstance( this ).CheckNetwork( this ) )// 网络未连接
		{
			CommonFunction.toastMsg( this , getString( R.string.network_req_failed ) );
			return;
		}
	...
	private void Download(final Face face , ViewHolder viewHolder )
	{
		...
		else
		{
			...
			new Thread(new Runnable( )
			{
				...
				public void run( )
				{
					FACE_DESCRIBE = BusinessHttpProtocol.getFaceDescribe( FaceMainActivityNew.this ,
							face.getFaceid( ) , FaceMainActivityNew.this );
					...
					try
					{
						manager = new FileDownloadManager( FaceMainActivityNew.this , callback ,
	...
	private void displayMyfaceBtn( final boolean isRefreshComplete )
	{
		if ( isRefreshComplete == true && mFaces.size( ) > 0 )
		{
			...
			mTitleRight.setTextColor( getResources( ).getColor( R.color.title_text ) );
		}
		else
		{
	...
	public void onClick( View v )
	{
		if (  v.equals( mTitleBack ) || v.equals(flLeft)  )
		{
			...
			Intent i = new Intent( FaceMainActivityNew.this , OwnFaceActivity.class );
...
//	{
//		return DataTag.VIEW_find_faceCenter;
//	}
}
