...
import net.iaround.tools.Model;
...
public class FaceCenterModel extends Model
{
								  ...
								  HttpCallBack callback )
	{
		...
		mark( flag , FaceCenterReqTypes.FaceCenterMainData );
		return flag;
	}
	...
	public long getOwnFaceData(Context context , HttpCallBack callback )
	{
		long flag = BusinessHttpProtocol.ownFaces( context , callback );
		mark( flag , FaceCenterReqTypes.OwnFacesData );
		return flag;
	}
...
//	}
	...
	private void getOwnFacesDataFromRes(JSONObject json , HashMap< String , Object > resMap )
	{
		...
		{
			int length = mapArray.length( );
			List<FaceManager.FaceLogoIcon> faces = new ArrayList<FaceManager.FaceLogoIcon>( );
			for ( int i = 0 ; i < length ; i++ )
			{
				JSONObject obj = JsonParseUtil.getJSONObject( mapArray , i );
				if ( obj != null )
	...
	public void reset()
	{
		...
		delFace.clear( );
		clearPrecentData( );
		FaceMainActivity.mFaces.clear( );
		FaceMainActivity.buyFace.clear( );
		FaceMainActivity.clickFace.clear( );
		FaceDetailActivity.faceList.clear( );
		FaceDetailActivityNew.faceList.clear();// yuchao
		FaceManager.mOwnGifFaces.clear( );
	}
