...
import net.iaround.model.entity.BaseBean;
...
public class RangeSeekBar<T extends Number> extends ImageView {
     ...
     */
    public static final int ACTIVE_COLOR = Color.rgb(255,64,100);
    /**
    ...
    private static final int DEFAULT_TEXT_DISTANCE_TO_BUTTON_IN_DP = 8;//长线上边距离
    private static final int DEFAULT_TEXT_DISTANCE_TO_TOP_IN_DP = 8;//长线下边距离

    private static final int LINE_HEIGHT_IN_DP = 12;
    ...
    private void init(Context context, AttributeSet attrs) {
        float barHeight;
        int thumbNormal = R.drawable.seek_thumb_disabled;
        int thumbPressed = R.drawable.seek_thumb_disabled;
        int thumbDisabled = R.drawable.seek_thumb_disabled;
        int thumbShadowColor;
        int defaultShadowColor = Color.argb(75, 0, 0, 0);
        int defaultShadowYOffset = CommonFunction.dipToPx(context, 2);
        int defaultShadowXOffset = CommonFunction.dipToPx(context, 0);
        int defaultShadowBlur = CommonFunction.dipToPx(context, 2);

        if (attrs == null) {
            setRangeToDefaultValues();
            internalPad = CommonFunction.dipToPx(context, INITIAL_PADDING_IN_DP);
            barHeight = CommonFunction.dipToPx(context, LINE_HEIGHT_IN_DP);
            activeColor = getContext().getResources().getColor(R.color.login_btn);
            defaultColor = getContext().getResources().getColor(R.color.login_btn);
            alwaysActive = false;
            ...
            try {
                ...
                defaultColor = a.getColor(R.styleable.RangeSeekBar_defaultColor, getContext().getResources().getColor(R.color.login_btn));
        ...
        textSize = CommonFunction.dipToPx(context, DEFAULT_TEXT_SIZE_IN_DP);
        distanceToTop = CommonFunction.dipToPx(context, DEFAULT_TEXT_DISTANCE_TO_TOP_IN_DP);
        textOffset = !showTextAboveThumbs ? 0 : this.textSize + CommonFunction.dipToPx(context,
    ...
    protected synchronized void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int height = thumbImage.getHeight()
                + (!showTextAboveThumbs ? 0 : CommonFunction.dipToPx(getContext(), HEIGHT_IN_DP))
...
//        canvas.drawRect(rect, paint);
        ...
        canvas.drawRoundRect(rect,CommonFunction.dipToPx(getContext(),30),CommonFunction.dipToPx(getContext(),30), paint);
...
//        if (showTextAboveThumbs && (activateOnDefaultValues || !selectedValuesAreDefault)) {
            ...
            if (!singleThumb) {
                ...
                int spacing = CommonFunction.dipToPx(getContext(), TEXT_LATERAL_PADDING_IN_DP);
