...
import net.iaround.ui.contacts.bean.FriendListBean.FriendsBean;
 ...
 */
public class ContactsAdapterFriend extends BaseAdapter {

    private Context mContext;
    private List<FriendsBean> mList;
    private ArrayList<User> userList;

    public ContactsAdapterFriend(Context context, List<FriendsBean> mList) {
        this.mContext = context;
        this.mList = mList;

    }

    public void updataList(List<FriendsBean> contactses) {
    ...
    public View getView(final int position, View convertView, ViewGroup viewGroup) {
        userList = FriendModel.getInstance(mContext).getFriendsList();
        ...
        if (convertView == null) {
            ...
            holder.constellation.setVisibility(View.GONE);//联系人界面中星座不可见
            convertView.setTag(holder);
        } else {
            holder = (ContactsHolder) convertView.getTag();
        }
        if (mList != null) {
            FriendsBean itemBean = mList.get(position);
            ...
            convertView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    for (User user : userList) {
                        if (mList.get(position).getUser().getUserid() == user.getUid()) {
                            user.setRelationship(1);//默认设置好友列表关系是 1 好友
